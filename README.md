...
* [dcp-tck](#dcp-tck)
    ...
    * [2. Set up the system-under-test](#2-set-up-the-system-under-test)
        ...
        * [2.4 Overriding the correlation ID for a single test](#24-overriding-the-correlation-id-for-a-single-test)
...
The following configuration is supported by the TCK:
...
| `dataspacetck.credentials.correlation.id` | the correlation ID for the issuance of the test credentials.<br>Some implementations may reject rogue (uncorrelated) `CredentialMessages`,<br>so this correlation ID provides a way to establish correlation. If omitted, a random UUID is generated by the TCK.<br>Can be overridden per test, see [2.4](#24-overriding-the-correlation-id-for-a-single-test) | `some-correlation-id`                                                    | no              |
| `dataspacetck.vc.scope.pattern`           | The regular expression used to extract the credential type from a requested scope.<br/>Must declare a named group `type`. The default pattern also captures an optional trailing segment in the named group `suffix`.                                           | `org[.]eclipse[.]dspace[.]dcp[.]vc[.]type:(?<type>[^:]+)(?<suffix>:.+)?` | no              |
| `dataspacetck.vc.scope.<credentialtype>`  | The scope used for the specified lowercase credential type. Defaults to `org.eclipse.dspace.dcp.vc.type:<CredentialType>`. Configure `membershipcredential` and `sensitivedatacredential` when different values should be sent.                                 | `org.eclipse.dspace.dcp.vc.type:MembershipCredential:read`               | no              |
...
The TCK hosts a CredentialService for the duration of the run, so the IssuerService under test must deliver the
credentials it issues to the TCK's Storage API. The `CredentialDeliveryTest` cases inspect that delivery - the message
envelope, the self-issued token it was sent with, and the credentials themselves - so an IssuerService that accepts a
`CredentialRequestMessage` but never delivers will fail them.
...
- **default scope**: by default, the TCK uses
  `org.eclipse.dspace.dcp.vc.type:MembershipCredential` as the Membership Credential scope. The default scope pattern
  also accepts optional trailing segments for compatibility with existing scope values.
...
The TCK supports the following configuration values:
...
| `dataspacetck.vc.scope.pattern`          | The regular expression used to extract the credential type from a requested scope.<br/>Must declare a named group `type`. The default pattern also captures an optional trailing segment in the named group `suffix`.                                   | `org[.]eclipse[.]dspace[.]dcp[.]vc[.]type:(?<type>[^:]+)(?<suffix>:.+)?` | no              |
| `dataspacetck.vc.scope.<credentialtype>` | The scope used for the specified lowercase credential type. Defaults to `org.eclipse.dspace.dcp.vc.type:<CredentialType>`. Configure `membershipcredential`, `sensitivedatacredential`, and `someothercredential` when different values should be sent. | `org.eclipse.dspace.dcp.vc.type:MembershipCredential:read`               | no              |

Test package: `"org.eclipse.dataspacetck.dcp.verification.presentation.verifier"`

When the verifier uses different scope values than the default, the TCK must be configured accordingly. The following
example shows how to configure the TCK for a verifier that uses different scope values for the Membership Credential and
the Sensitive Data Credential. The `someothercredential` value is only used by the verifier tests.

```properties
dataspacetck.vc.scope.membershipcredential=org.eclipse.dspace.dcp.vc.type:MembershipCredential:read
dataspacetck.vc.scope.sensitivedatacredential=org.eclipse.dspace.dcp.vc.type:SensitiveDataCredential:read
dataspacetck.vc.scope.someothercredential=org.eclipse.dspace.dcp.vc.type:SomeOtherCredential:read
```

If the scope alias or grammar is also different, `dataspacetck.vc.scope.pattern` must be configured with a corresponding
expression that extracts the credential type through its named `type` group.

### 2.4 Overriding the correlation ID for a single test

The holder PID the TCK sends is a correlation ID: it identifies a credential request the system under test already
knows about. A SUT that closes a request once it has been fulfilled, or that pre-provisions a fixed set of
correlation IDs, cannot serve every test from the single `dataspacetck.credentials.correlation.id` value.

Any test can therefore be given its own correlation ID by suffixing that property with the **test method name**:

```properties
# used by any test with no override
dataspacetck.credentials.correlation.id=foobar

# per-test overrides
dataspacetck.credentials.correlation.id.cs_06_05_01_credentialMessage_rejectedStatus=abc-123
dataspacetck.credentials.correlation.id.cs_06_05_02_credentialMessage_subjectNotHolder=def-456
```

Resolution order for a given test is: the test-specific property, then `dataspacetck.credentials.correlation.id`,
then a random UUID. No test needs to opt in — the override applies wherever the TCK uses the holder PID, including
the credentials it pre-loads into the SUT before a test runs.

Two things to note:

- The key is the **method** name (`cs_06_05_01_credentialMessage_rejectedStatus`), not the test's display name.
  Method names are unique across the whole suite, so no class qualifier is needed.
- Per-test overrides must be supplied through the properties file or as a system property. They **cannot** be set
  as environment variables: the TCK maps env var names to properties by lowercasing and replacing every `_` with a
  `.`, which would corrupt the underscores in a test method name.
...
void runVppTestsWithDocker() {
    var config = Map.of(
            // insert your required configuration here, check in previous chapters
    );
...
The example above presumes that the TCK's callback endpoint is reachable at port `8080`. This is also the mapped docker
port.
Furthermore, according to the configuration shown above, the TCK expects to find the verifier's DID document at
`http://localhost:9876/verifier/did.json`.
